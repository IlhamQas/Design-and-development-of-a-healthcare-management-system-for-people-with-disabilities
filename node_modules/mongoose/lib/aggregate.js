...
Aggregate.prototype.graphLookup = function(options) {
  console.log('\n=== DEBUG: graphLookup() called ===');
  console.log('Full options object received:', options);
  
  const cloneOptions = {};
  
  if (options) {
    if (!utils.isObject(options)) {
      console.log('Invalid options (not an object):', options);
      throw new TypeError('Invalid graphLookup() argument. Must be an object.');
    }

    utils.mergeClone(cloneOptions, options);
    const startWith = cloneOptions.startWith;
    console.log('startWith value:', startWith, 'Type:', typeof startWith);
    
    if (startWith && typeof startWith === 'string') {
      console.log('Processing startWith string:', startWith);
      cloneOptions.startWith = cloneOptions.startWith.startsWith('$') ?
        cloneOptions.startWith :
        '$' + cloneOptions.startWith;
    } else if (startWith !== undefined) {
      console.log('Unexpected startWith type:', {
        value: startWith,
        type: typeof startWith
      });
    }
  } else {
    console.log('graphLookup called with undefined/null options!');
  }

  console.log('Final cloneOptions:', cloneOptions);
